...
function SetOrder() {
  ...
  if (!token || !userId) {
    return (
      <p className="text-center p-8">من فضلك قم بتسجيل الدخول لإكمال الطلب</p>
    );
  }
  ...
  const handleConfirmOrder = async (e) => {
    ...
    if (paymentMethod === "Wallet" && !walletData) {
      return setWarning("من فضلك اختر المحفظة وأدخل رقم الهاتف لإتمام الدفع");
    }
            ...
            {cartLoading ? (
              ...
              <>
                ...
                <p className="text-sm text-gray-700 text-right">
                  عنوان التوصيل: {location || "لم يتم إدخاله بعد"}
                </p>
                <p className="text-sm text-gray-700 text-right">
                  طريقة الدفع:{" "}
                  {paymentMethod === "Cash"
